/**
 * server/aai/async.ts - AssemblyAI pre-recorded (async) transcription + Speech Understanding client.
 * Promoted verbatim from spikes/async/client.ts (DESIGN §3.2 fix 5.3-3: it now lives inside the root tsconfig).
 *
 * Dependency-free (global fetch). Server-side only: never ship the key to a browser.
 *
 * Covers: POST /v2/upload, POST /v2/transcript, GET /v2/transcript/{id} (poll), /sentences, /paragraphs,
 * /redacted-audio, /word-search, /srt|/vtt, DELETE, plus the Speech Understanding endpoint
 * POST https://llm-gateway.assemblyai.com/v1/understanding, and webhook helpers.
 *
 * Shapes are typed from live responses captured 2026-09-24 (research/10c). Auth: `Authorization: <key>` (raw key).
 * Baton uses it for F3 (verify_takeover: VA recording -> multichannel U3.5 Pro, ch1 = user, ch2 = agent) and labels.
 */
import "server-only";
