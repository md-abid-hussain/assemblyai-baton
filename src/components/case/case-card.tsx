"use client";
/**
 * CaseCard (DESIGN §1.4 S2 center, PLATFORM §7.6): header, ReadinessGauge, FieldRow per field, ConflictCard,
 * AI-confirmed tags.
 *
 * Everything relay-shaped comes from the run's `UiSpec` (labels, groups, required set, title): the same component
 * renders Baton's 10 required facts and a Dental booking's 4. Without a spec it falls back to `BATON_UI_SPEC`, so
 * fixture logs and recorded bundles render exactly as before.
 */
...
import { caseGroups, fieldLabels, names } from "@/client/store/selectors";
import { caseTitleOf, requiredOf, specOf } from "@/client/store/ui-spec";
...
export function ReadinessGauge({ fields, required }: { fields: Record<FieldId, FieldState> | null; required: readonly string[] }) {
  const segs = (required.length ? required : ["required"]).map((f) => {
    const st = fields?.[f as FieldId];
    return st ? chipKindOf(st) : "MISSING";
  });
...
const FieldRow = memo(function FieldRow({ f, label, required, ai, disagree, agrees, who }: { f: FieldState; label: string; required: boolean; ai: boolean; disagree: boolean; agrees: boolean; who: { rep: string; customer: string } }) {
        ...
        <div className="flex w-[92px] items-center justify-end gap-1">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button type="button" className="rounded-full focus-visible:ring-2 focus-visible:ring-(--ai) focus-visible:outline-none" aria-label={`${label}: ${kind.toLowerCase()}, ${reason}`}>
...
export function ConflictCard({ c, who, label }: { c: ConflictCardT; who: { rep: string; customer: string }; label: string }) {
  const partyName = (p: string) => (p === "rep" ? who.rep : p === "customer" ? who.customer : p === "ai" ? "the AI" : p);
  const [a, b] = c.values;
  return (
    <div role="group" aria-label={`Conflict on ${label}`} className={cn("rounded-lg border p-3", c.resolved ? "border-(--bt-line) bg-(--bt-panel-2)" : "border-(--conflict)/50 bg-(--conflict-bg)")}>
      <div className="flex items-center gap-2 text-sm font-semibold text-(--conflict-fg)">
        <AlertTriangleIcon className="size-4" aria-hidden="true" />
        {c.resolved ? "Resolved conflict" : "Conflict"} · {label}
      </div>
...
export function CaseCard() {
  ...
  const groups = useBaton(caseGroups, sameGroups);
  const label = fieldLabels({ relay: spec });
  const requiredIds = requiredOf(spec);
  const requiredSet = new Set(requiredIds);
  const p = ctx?.policy;
  const multiGroup = groups.filter((g) => g.group).length > 1;
  return (
    <section aria-labelledby="case-h" className="flex h-full min-h-0 flex-col">
      <header className="flex items-start justify-between gap-3 border-b border-(--bt-line) px-4 pt-3 pb-3">
        <div className="min-w-0">
          ...
          <div className="bt-display text-xl leading-tight font-bold">{caseTitleOf(spec)}</div>
          {p && spec.relay.flagship ? (
            ...
            <div className="mt-0.5 truncate text-xs text-(--bt-muted)">
              {who.customerFull} · {who.org}
            </div>
          )}
        </div>
      ...
      <div className="bt-scroll min-h-0 flex-1 px-2 py-2">
        {cs?.conflicts.length ? (
          <div className="mb-2 space-y-2 px-2">
            {cs.conflicts.map((c) => (
              <ConflictCard key={c.field} c={c} who={who} label={label(c.field)} />
            ))}
          </div>
        ) : null}
        {groups.length ? (
          <ul aria-label="Case facts" className="space-y-0.5">
            {groups.map((g) => (
              <li key={g.group ?? "_"}>
                {multiGroup && g.group ? (
                  <h3 className="bt-eyebrow sticky top-0 z-10 bg-(--bt-panel) px-2 pt-2 pb-0.5 text-(--bt-faint)">{g.group}</h3>
                ) : null}
                <ul className="space-y-0.5">
                  {g.rows.map((f) => (
                    <FieldRow
                      ...
                      f={f}
                      label={label(f.field)}
                      required={requiredSet.has(f.field)}
                      ai={ai.includes(f.field)}
                      disagree={!!verifier?.disagreements.includes(f.field)}
                      agrees={!!verifier?.agrees}
                      who={who}
                    />
                  ))}
                </ul>
          ...
          <EmptyCase ids={requiredIds} label={label} />
        )}
      </div>
    </section>
  );
...
const sameGroups = (a: ReturnType<typeof caseGroups>, b: ReturnType<typeof caseGroups>): boolean =>
  a.length === b.length && a.every((g, i) => g.group === b[i]?.group && g.rows.length === b[i]?.rows.length && g.rows.every((r, j) => r === b[i]?.rows[j]));

function EmptyCase({ ids, label }: { ids: readonly string[]; label: (id: string) => string }) {
  return (
    <ul aria-label="Case facts (waiting for the call)" className="space-y-1 px-2 py-1">
      {ids.map((f) => (
        <li key={f} className="flex items-center gap-3 rounded-lg px-2 py-2">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[11px] text-(--bt-muted)">{label(f)}</div>
